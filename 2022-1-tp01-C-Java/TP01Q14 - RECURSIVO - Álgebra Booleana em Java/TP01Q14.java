...
class TP01Q14 {
   ...
   public static String solve(String booleanExp, int[] vars) {

      if (booleanExp.contains("(")) {
         ...
         booleanExp = solve(booleanExp, vars);
      }
   ...
   public static void main (String[] args){
      ...
      for(int i = 0; i < numEntrada; i++){
         int varCount = entrada[i].charAt(0) - '0';
         int[] vars = new int[varCount];

         for (int j = 0; j < varCount; j++)
            vars[j] = entrada[i].charAt((j * 2) + 2) - '0';

         entrada[i] = entrada[i].substring((varCount + 1) * 2);
         System.out.println(solve(entrada[i], vars));
      }
      sc.close();
   }
}
