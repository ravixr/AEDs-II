...
template <typename T>
void BinaryTree<T>::RemoveByKey(std::string& key) {
...
template <typename T>
void BinaryTree<T>::RemoveByKey(Node* node, std::string& key) {
    if (node == NULL) {
        return;
    } else {
        if (node->data == key)
...
int main() {
    BinaryTree<Movie> binary_tree;
    std::string s;
    std::getline(std::cin, s);
    while (s.compare("FIM") != 0) {
        binary_tree.Insert(Movie::ReadMovie(s));
        std::getline(std::cin, s);
    }
    int count = 0;
    std::getline(std::cin, s);
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (s.find("I ") != std::string::npos) {
            ...
            s = s.substr(2);
            binary_tree.RemoveByKey(s);
        }
    }
    std::getline(std::cin, s);
    while (s.compare("FIM") != 0) {
        std::cout << s << "\n";
        binary_tree.Search(s) ? std::cout << "SIM\n" : std::cout << "NAO\n";
        std::getline(std::cin, s);
    }
}