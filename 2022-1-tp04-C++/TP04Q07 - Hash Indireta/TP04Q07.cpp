...
public:
    ...
    // Movie operator overload to compare with original title as a strin
...
public:
    ...
    bool Insert(T data, const std::string& data_string) {
        bool res = false;
            table[ToHash(data_string)].push_back(data);
            res = true;
        return res;
    }
...
int main() {
    std::string s;
    DynamicHashTable<Movie> hash_table;
