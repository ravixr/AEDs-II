...
class MovieList {
    ...
    void quicksortByStatus(int left, int right) {
		Movie temp;
        int i = left, j = right;
...
class TP03Q06 {
	...
	public static void main (String[] args) throws Exception {
        ...
        mList.quicksortByStatus(0, mList.movies.length - 1);
        duration = (System.nanoTime() - duration);
		for(int i = 0; i < inputLength[0]; i++){
			Movie.printMovie(mList.movies[i]);
		}
        printLogFile("751441_quicksort.txt", MovieList.numComp, MovieList.numMov, duration);
