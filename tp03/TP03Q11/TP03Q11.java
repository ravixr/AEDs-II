...
class MovieLinkedStack {
    public Cell top;
    public int size;

    public MovieLinkedStack() {
        top = new Cell();
        size = 0;
    }
...
class TP03Q11 {
    ...
    public static void main (String[] args) throws IOException {
        ...
        MovieLinkedStack mStack = new MovieLinkedStack();
        ...
        for(int i = 0; i < inputLength[1]; i++) {
            if (input[i].startsWith("I"))
                ...
                try {
                    MyIO.println("(R) " + Movie.class.cast(mStack.pop()).getName());
                } catch (Exception e) {
                    e.printStackTrace();
                }
        }

        mStack.print(mStack.top, 0);

        /*int i = 0;
        for(Cell tmp = mStack.top; i < mStack.size; tmp = tmp.next, i++) {
            MyIO.print("[" + i + "] ");
