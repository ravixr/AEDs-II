...
class CircularLinkedQueue {
    ...
    public int insert(Object obj) throws Exception {
        ...
        float aux = 0;
        int i = 0;
        for(Cell j = start.next; i < size; j = j.next, i++) {
            aux += Movie.class.cast(j.element).getRuntime();
        }
...
class TP03Q12 {
    ...
    public static void main (String[] args) throws Exception {
        ...
        CircularLinkedQueue mCQueue = new CircularLinkedQueue();
        ...
        for(int i = 0; i < inputLength[1]; i++) {
            if (input[i].startsWith("I"))
                ...
                MyIO.println("(R) " + Movie.class.cast(mCQueue.remove()).getName());
        }
        int i = 0;
        for(Cell tmp = mCQueue.start.next; i < mCQueue.size; tmp = tmp.next, i++) {
            MyIO.print("[" + i + "] ");
            Movie.printMovie(Movie.class.cast(tmp.element));
        }
