...
template <typename T>
void AvlTree<T>::PrintPreOrder(Node* node) const {
    if (node != nullptr) {
        std::cout << node->data << ", ";
...
template <typename T>
void AvlTree<T>::PrintPostOrder(Node* node) const {
    if (node != nullptr) {
...
template <typename T>
int AvlTree<T>::Size(Node* node) const {
    int size = 1;
    if (node != nullptr) {
...
template <typename T>
bool AvlTree<T>::Search(std::string& key) const {
    std::cout << "raiz ";
    return Search(this->m_root, key);
}

template <typename T>
bool AvlTree<T>::Search(Node* node, std::string& key) const {
    if (node == nullptr) {
        return false;
    }
    else {
