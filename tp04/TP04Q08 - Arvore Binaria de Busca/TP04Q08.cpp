...
public:
    ...
    void PrintPreOrder() {
        std::string s(PrintPreOrder(this->m_root));
        s.pop_back();
        std::cout << s << "\n";
    }

    void PrintInOrder() {
        std::string s(PrintInOrder(this->m_root));
        s.pop_back();
        std::cout << s << "\n";
    }

	void PrintPostOrder() {
        std::string s(PrintPostOrder(this->m_root));
        s.pop_back();
        std::cout << s << "\n";
    }
...
template <typename T>
std::string BinaryTree<T>::PrintPreOrder(Node* node) {
    if (node != NULL) {
        return std::to_string(node->data).append(" ").append(PrintPreOrder(node->left)).append(PrintPreOrder(node->right));
    }
    return "";
}

template <typename T>
std::string BinaryTree<T>::PrintInOrder(Node* node) {
    if (node != NULL) {
        return PrintInOrder(node->left).append(std::to_string(node->data)).append(" ").append(PrintInOrder(node->right));
    }
    return "";
}

template <typename T>
std::string BinaryTree<T>::PrintPostOrder(Node* node){
	if (node != NULL) {
		return PrintPostOrder(node->left).append(PrintPostOrder(node->right)).append(std::to_string(node->data)).append(" ");
	}
    return "";
}
...
template <typename T>
bool BinaryTree<T>::Search(std::string& key) {
...
template <typename T>
bool BinaryTree<T>::Search(Node* node, std::string& key) {
    if (node == NULL) {
        return false;
    } else {
...
int main() {
	...
	for (int i = 0; i < count_; i++) {
		int count_insert = 0;
		std::cin >> count_insert;
		BinaryTree<int> binary_tree;
		for (int j = 0; j < count_insert; j++) {
			int tmp = 0;
			std::cin >> tmp;
			binary_tree.Insert(tmp);
		}
		std::cout << "Case " << i + 1 << ":\n";
		std::cout << "Pre.: "; binary_tree.PrintPreOrder();
		std::cout << "In..: "; binary_tree.PrintInOrder();
		std::cout << "Post: "; binary_tree.PrintPostOrder();
        std::cout << "\n";
	}
}